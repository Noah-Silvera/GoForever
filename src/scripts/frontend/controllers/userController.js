 define(['controllers/controller','views/UserView','models/UserModel','lib/request','RequestHandler','lib/Statistics'], function(Controller,UserView,UserModel,request,RequestHandler,Stats){
    ...
    class UserController extends Controller{
        ...
        signup(){
            ...
            if (data.username.length == 0) alert("Must provide a username.");
                ...
                RequestHandler.signup(data)
                    .then(function(res){
                        window.location.href = 'http://roberts.seng.uvic.ca/userLanding.html'
                    })
                    .catch(function(err){
                        console.debug(err)
                    })   
        ...
        logout(){
            ...
            .then(function(res){
                console.log(res)
                window.location.href = 'http://roberts.seng.uvic.ca/'
            })
            .catch(function(err){
                console.log(err)
            })
        ...
        playGame(){
            ...
            window.location.href = 'http://roberts.seng.uvic.ca/gameOptions'
        }
        ...
        loadProfilePage(){
            window.location.href = 'http://roberts.seng.uvic.ca/userProfile.html'
        }

        loadSettingsPage(){
            // do user specific things here

            window.location.href = 'http://roberts.seng.uvic.ca/userSettings.html'
        }
